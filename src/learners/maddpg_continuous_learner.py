...
class MADDPGContinuousLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        actions = batch["actions"]  # (bs, T+1, n_agents, n_actions)
        ...
        actions_flat = (
            actions.view(batch_size, -1, 1, self.n_agents * self.n_actions)
            .expand(-1, -1, self.n_agents, -1)
        )
        q_taken = self.critic(inputs[:, :-1], actions_flat[:, :-1].detach()).view(
            batch_size, -1, 1
        )
        ...
        for t in range(1, batch.max_seq_length):
            target_actions.append(self.target_mac.target_actions(batch, t))
        target_actions = th.stack(target_actions, dim=1)
        target_actions_flat = (
            target_actions.view(batch_size, -1, 1, self.n_agents * self.n_actions)
            .expand(-1, -1, self.n_agents, -1)
        )
        target_vals = self.target_critic(inputs[:, 1:], target_actions_flat.detach()).view(
            batch_size, -1, 1
        )
        ...
        step_lr_scheduler(self.critic_lr_scheduler, critic_loss.item())

        self.mac.init_hidden(batch_size)
        pi_actions = []
        for t in range(batch.max_seq_length - 1):
            pi_actions.append(self.mac.forward(batch, t=t))
        pi_actions = th.stack(pi_actions, dim=1)  # (bs, T, n_agents, n_actions)
        pi_actions_flat = (
            pi_actions.view(batch_size, -1, 1, self.n_agents * self.n_actions)
            .expand(-1, -1, self.n_agents, -1)
        )

        per_agent_joint_actions = []
        for i in range(self.n_agents):
            split_actions = th.split(pi_actions_flat[:, :, i, :], self.n_actions, dim=2)
            chosen = []
            for j in range(self.n_agents):
                if i == j:
                    chosen.append(split_actions[j])
                else:
                    chosen.append(split_actions[j].detach())
            per_agent_joint_actions.append(th.cat(chosen, dim=-1).unsqueeze(2))
        actor_actions = th.cat(per_agent_joint_actions, dim=2)

        q = self.critic(inputs[:, :-1], actor_actions).reshape(-1, 1)
        mask_flat = mask.reshape(-1, 1)
        reg = float(getattr(self.args, "reg", 0.0))
        action_reg = (pi_actions**2).mean()
        pg_loss = -(q * mask_flat).mean() + reg * action_reg
    ...
    def _build_inputs(self, batch, t=None):
        ...
        if self.args.obs_last_action:
            if t == 0:
                ...
                last_actions = th.cat(
                    [
                        th.zeros_like(batch["actions"][:, 0:1]),
                        batch["actions"][:, :-1],
                    ],
                    dim=1,
                )
